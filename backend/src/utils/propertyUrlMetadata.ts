export interface PropertyUrlMetadata {
  title: string
  price: number | null
...
function humanizeSlug(value: string) {
  return value
    ...
    .map((word, index) => {
      ...
      if (ACCENTED_WORDS[normalized]) return ACCENTED_WORDS[normalized]
...
function detailsToMetadata(details: PropertyDetails, source: string): PropertyUrlMetadata {
...
function parseVivaReal(slug: string, source: string): PropertyUrlMetadata | null {
  const prefix = slug.match(/^(?<type>[a-z]+)-(?<rooms>\d+)-quartos-(?<neighborhood>.+?)-bairros-(?<tail>.+)$/i)
  const prefixGroups = prefix?.groups
  if (!prefixGroups?.type || !prefixGroups.rooms || !prefixGroups.neighborhood || !prefixGroups.tail) return null

  const tail = prefixGroups.tail.match(
    /^(?<cityAndFeatures>.+)-(?<area>\d+(?:[.,]\d+)?)m2-(?:venda|aluguel)-rs(?<price>\d+)-id-\d+$/i,
  )
  const tailGroups = tail?.groups
  if (!tailGroups?.cityAndFeatures || !tailGroups.area || !tailGroups.price) return null
...
function parseZap(slug: string, source: string): PropertyUrlMetadata | null {
  const match = slug.match(
    /^(?:venda|aluguel)-(?<type>[a-z]+)-(?<rooms>\d+)-quartos-(?<place>.+)-porto-alegre-rs-(?<area>\d+(?:[.,]\d+)?)m2-id-\d+$/i,
  )
  const groups = match?.groups
  if (!groups?.type || !groups.rooms || !groups.place || !groups.area) return null
...
function parseImovelWeb(slug: string, source: string): PropertyUrlMetadata | null {
  const locationFirst = slug.match(
    /^(?<type>[a-z]+)-a-venda-(?<neighborhood>.+)-(?<rooms>\d+)-quartos-(?<area>\d+(?:[.,]\d+)?)-\d+$/i,
  )
  ...
  const roomsFirst = slug.match(
    /^(?<type>[a-z]+)-(?<rooms>\d+)-dormitorios-(?<area>\d+(?:[.,]\d+)?)-m-em-(?<neighborhood>.+)-\d+$/i,
  )
...
function parseCreditoReal(slug: string, source: string): PropertyUrlMetadata | null {
  const match = slug.match(
    /^(?<type>[a-z]+)-em-(?<neighborhood>.+)-porto-alegre-ag-\d+-cod-\d+$/i,
  )
...
function parseWimoveis(slug: string, source: string): PropertyUrlMetadata | null {
  const match = slug.match(/^(?<rooms>\d+)-dormitorios-(?<location>.+)-\d+$/i)
  const groups = match?.groups
  if (!groups?.rooms || !groups.location) return null
...
function parseGenericSlug(slug: string, source: string): PropertyUrlMetadata {
  const cleaned = slug
    .replace(/\.html?$/i, '')
    .replace(/-(?:id-)?\d{6,}$/i, '')
...
export function extractPropertyUrlMetadata(rawUrl: string): PropertyUrlMetadata {
  ...
  if (source.endsWith('vivareal.com.br')) return parseVivaReal(slug, source) ?? parseGenericSlug(slug, source)
  if (source.endsWith('zapimoveis.com.br')) return parseZap(slug, source) ?? parseGenericSlug(slug, source)
  if (source.endsWith('imovelweb.com.br')) return parseImovelWeb(slug.replace(/\.html?$/i, ''), source) ?? parseGenericSlug(slug, source)
  if (source.endsWith('creditoreal.com.br')) return parseCreditoReal(slug, source) ?? parseGenericSlug(slug, source)
  if (source.endsWith('wimoveis.com.br')) return parseWimoveis(slug.replace(/\.html?$/i, ''), source) ?? parseGenericSlug(slug, source)
