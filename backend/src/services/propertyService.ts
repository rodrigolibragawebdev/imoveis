...
function enrichFromUrl(property: Property): Property {
...
class PropertyService {
  ...
  async createMany(links: string[]) {
    const uniqueLinks = [...new Set(links)]
    const results = new Array<ReturnType<typeof propertyRepository.create>>(uniqueLinks.length)
    let nextIndex = 0
  ...
  updateReview(id: number, rating: PropertyRating, note: string) {
  ...
  delete(id: number) {
    if (!propertyRepository.delete(id)) throw new AppError('Imóvel não encontrado', 404)
  }
}
