...
class FurnitureService {
  ...
  listItems(categoryId: number | undefined, sort: 'price-asc' | 'price-desc' | 'newest') {
    return furnitureRepository.findItems({ categoryId, sort })
  }

  async createItem(input: CreateFurnitureInput) {
  ...
  deleteItem(id: number) {
    if (!furnitureRepository.deleteItem(id)) throw new AppError('Item não encontrado', 404)
  }
}
