<template>
	<section class="max-w-7xl mx-auto">
		<div
			class="flex flex-column lg:flex-row lg:align-items-end justify-content-between gap-4 mb-5 md:mb-6"
		>
			<div class="hidden md:block">
				<span
					class="inline-block text-sm uppercase font-bold text-terracotta mb-2 eyebrow"
					>Do essencial ao detalhe</span
				>
				<h1 class="display-font text-5xl md:text-7xl line-height-1 m-0">
					A casa,<br /><em>peça por peça.</em>
				</h1>
			</div>
		...
		<div
			class="surface-card border-1 border-round-3xl p-3 md:p-4 catalog-panel"
		>
			<FurnitureFilters
			...
			<div v-if="store.loading" class="grid">
				...
				>
					<Skeleton height="25rem" border-radius="1rem" />
				</div>
			</div>
			...
			<div v-else class="text-center py-7">
				...
				<p class="opacity-60">
					Troque o filtro ou adicione o primeiro link desta categoria.
				</p>
			</div>
		</div>
...
async function saveItem(payload: FurnitureItemInput) {
	try {
		...
		toast.add({
			...
			detail: wasEditing
				? "As alterações já aparecem no card."
				: "O card já está na lista.",
			life: 2800,
		})
	} catch {
		toast.add({
...
.catalog-panel {
	...
	box-shadow: 0 18px 54px rgba(54, 82, 68, 0.07);
}
</style>
